...
job-tracking model: it waits until the batch worker drops ``energy.txt`` into
the folder, then exits, freeing the slot.  USPEX polls the job's exit code:
still running -> IN_PROGRESS, exit 0 -> FINISHED, non-zero -> FAILED.  So this
stub must stay alive until ``energy.txt`` and then exit 0.
...
def main():
    cf = Path.cwd().name
    print(f"[{time.strftime('%H:%M:%S')}] {cf}: waiting for batch worker (energy.txt)...",
          flush=True)
    while not os.path.isfile("energy.txt"):
        time.sleep(2)
    print(f"[{time.strftime('%H:%M:%S')}] {cf}: energy.txt present, exiting.", flush=True)
