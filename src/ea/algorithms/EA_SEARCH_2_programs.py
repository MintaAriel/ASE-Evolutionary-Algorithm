...
def sym_random_generator(blocks):
    elements, counts = zip(*blocks)
    while True:
        xtal = pyxtal()
        try:
            ...
            xtal.from_random(dim=3,
                             ...
                             random_state=1)
            atom = xtal.to_ase()
            atom.symbols = ''.join(f'{el}{count}' for el, count in blocks)
...
class Genetic_algorith():
    ...
    def start(self):
        ...
        while len(all_max_indices) <  max_generations and gen < 60:
            ...
            #Create new random symmetric structures
            fracRand = []

            for i in range(n_samples_sym):
                sym_struc = self.relax.use_gulp_no_add(sym_random_generator(blocks=[('Mg', 4), ('Al', 8), ('O', 16)]))
