...
class Genetic_algorith():
    def __init__(self, database):
        self.db = database
        self.work_dir = os.path.dirname(self.db)
        ...
        self.blmin = closest_distances_generator(atom_numbers=[1,6,7, 8],  ratio_of_covalent_radii=0.5)
        self.relax = Gulp_relaxation(self.work_dir, self.db)
    ...
    def start(self):
        ...
        blmin_soft = closest_distances_generator(atom_numbers=[1,6,7, 8],  ratio_of_covalent_radii=0.1)
        softmut = SoftMutation(blmin_soft, bounds=[2.0, 5.0], used_modes_file=dir+'/used_modes.json', use_tags=True)
        ...
        operator_ratio = [6.0, 3.0, 3.0]
        operators = OperationSelector(operator_ratio, [pairing, softmut, strainmut])

        print('========START=========')
        print(f'Parameters: \nCell Bound: {cellbounds.bounds} \n\nOperators: \n   Ratio-{operator_ratio} \n   OP_list-{operators.oplist}')
        ...
        while len(all_max_indices) < max_generations and gen < 60:
            ...
            for step in range(population_size):
                ...
                while a3 is None:
                    ...
                    mom = a1.info['confid']
                    dad = a2.info['confid']
                    print(f"Using parents {mom} and {dad} ")
                ...
                if 'pairing' in desc:
                    a3.info['key_value_pairs']['dad'] = dad
                else:
                    None

                a3.info['key_value_pairs']['mom'] = mom
...
experiment = Genetic_algorith('/home/vito/PythonProjects/ASEProject/EA/test/struc-gen/theophilline.db')
experiment.start()
